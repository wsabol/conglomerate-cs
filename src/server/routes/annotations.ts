...
// List memories for a target (event or media).
route.get("/", async (c) => {
...
route.post("/", requireUser, async (c) => {
  ...
  const dto = await getAnnotationById(db, inserted.id);
...
route.patch("/:id", requireUser, async (c) => {
  ...
  const dto = await getAnnotationById(db, id);
...
route.delete("/:id", requireUser, async (c) => {
  ...
  const userId = await resolveUserId(db, user);
  // Authors may delete their own; editors may delete any (confirmed decision).
  if (user.role !== "editor" && existing.authorId !== userId) {
